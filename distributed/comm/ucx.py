...
ADDRESS = config.get("distributed.comm.ucxaddress", DEFAULT_ADDRESS)
...
class UCX(Comm):
    ...
    def __init__(self, ep: ucp.ucp_py_ep, address: str, deserialize=True):
...
class UCXConnector(Connector):
    ...
    async def connect(self, address: str, deserialize=True, **connection_args) -> UCX:
        ...
        return self.comm_class(ep, self.prefix + address, deserialize=deserialize)
